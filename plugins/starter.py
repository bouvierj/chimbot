...
p_bot_key = re.compile("Reminder:[\s]chimbot[\s]*google[\s]*API[\s]*key", re.I)
...
def process_message(data):
    ...
    if p_bot_hi.match(data['text']):
 	...
 	outputs.append([data['channel'], "You input: " + googleapikey])
    	...
    	if re.match(r"\d*d(?i)(\d+|f(?i)udge|f(?i)ate)$", rollinfo):
    		numdie = re.sub("d(?i)(\d+|f(?i)udge|f(?i)ate)", '', rollinfo)
    		if numdie == '':
    			numdie = "1"
	...
	try:
    	...
    	if data['user'].startswith("U0KH7TRCG") or data['user'].startswith("U0KHAR1LH") or data['user'].startswith("U0KH6S3QR"):
